...
export default function ReportForm() {
  ...
  const [showReportTypeConfirmation, setShowReportTypeConfirmation] =
    useState(false);
    ...
    allReportsTypes.current.forEach((reportType) => {
      ...
      if (metaData && !Object.keys(metaData).includes("type")) {
        if (reportType.type === "income")
          incomeReports.push({ value: reportType.id, label: reportType.name });
        if (reportType.type === "expense")
          outcomeReports.push({ value: reportType.id, label: reportType.name });
      }
    });
  ...
  const setReportByRole = () => {
    ...
    if (session.role_id === 4) {
      ...
      setReports([{ value: 3, label: "Proveedor" }]);
    ...
    if (session.role_id === 5) {
      ...
      setReports([{ value: 4, label: "Depositante" }]);
    ...
    if (session.role_id === 6) {
      ...
      setReports([{ value: 5, label: "Caja fuerte" }]);
      ...
      try {
        ...
        if (showCountries) {
          ...
          if (countriesResponse && countriesResponse.data) {
            const countriesOptions = countriesResponse.data.map(
              ({ name, shortcode, currency, id }) => {
                return {
            ...
            setCountry(
              countriesOptions.find(({ value }) => value == session.country_id),
            );
            setReports([
              { value: 1, label: "Reporte Tipo 1" },
              { value: 2, label: "Reporte Tipo 2" },
  ...
  const getReportTypesByReport = (report = 0) => {
    ...
    const filteredReports = intlReportTypes.current.filter(({ meta_data }) => {
    ...
    if (report == 4) {
      const expenseReports = filteredReports
        .slice(1)
      ...
      if (headColumns.length === 0) {
        for (let key of formData.keys()) {
          if (reportsColumnsMap.has(key))
            headColumns.push(reportsColumnsMap.get(key));
        }
      }
    ...
    <>
      <section className="container p-2 mb-2">
        <div className="WelcomeContainer pb-2">
          <h6 className="welcome">
            ...
            <span className="text-dark fs-5 fw-bold">+</span>
          </h6>
          <div className="d-flex align-items-center justify-content-between">
            ...
            <button
              ...
              disabled={tableData.body.length === 0 || isLoading}
              onClick={postReport}
            >
              Guardar +
            </button>
      ...
      <section className="container card border border-0 rounded p-4 pb-2">
        ...
        <ReportTableContext.Provider
          value={{ handleSubmit, setError, country }}
        >
          <div className="py-4">
            {reportType &&
              componentsMap.has(reportType.value) &&
              componentsMap.get(reportType.value)}
          </div>
        </ReportTableContext.Provider>
        ...
        {tableData && tableData.header.length > 0 && (
          <div className="w-100 overflow-hidden overflow-x-auto border rounded mb-4">
            <table className="m-0 table table-striped">
              <thead>
                <tr>
                  {tableData.header.map((header, hIndex) => (
