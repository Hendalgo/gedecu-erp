...
export default function BankAccountForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setBanks(
        banksResponse.map(({ id, name, country, type }) => ({
          label: `${name} - ${country.name} (${type.name})`,
          value: id,
        })),
      );
      setCurrencies(
        currenciesResponse.map(({ id, name, shortcode }) => ({
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      if (!form.get("identifier"))
        errors.push("El campo Identificador es obligatorio");
      if (!form.get("bank_id")) errors.push("El campo Banco es obligatorio");
      if (!form.get("currency_id"))
        errors.push("El campo Moneda es obligatorio");
      if (!form.get("balance"))
        errors.push("El campo Monto inicial es obligatorio");
      ...
      if (response.status === 201) {
        setAlert({
          messages: ["Cuenta registrada exitosamente"],
          variant: "success",
        });
    ...
    <>
      <section className="p-3">
        ...
        <span className="ModalTopSubTitle">
          Esta pestaña le permite registrar una nueva cuenta bancaria o de
          alguna otra plataforma monetaria al local asignado.
        </span>
      </section>
          ...
          <div className="row mb-3">
            ...
            <div className="col">
              ...
              <input
                ...
                id="identifier"
                className="form-control"
                placeholder="00000000; mail@example.com"
              />
            </div>
            <div className="col">
