...
const Users = () => {
  ...
  const handleType = (e)=>{
    getUsers(`order=created_at&order_by=desc&role=${e}&search=${form.current.search.value}`).then(r => setUsers(r));
  }
  const handleSearch = (e)=>{
    e.preventDefault();
    if (form.current.search !== '') {
      getUsers(`order=created_at&order_by=desc&search=${form.current.search.value}`).then(r=> setUsers(r));
    }
  }
  const handleChange = (offset)=>{
    getUsers(`order=created_at&order_by=desc&page=${offset.selected+1}&search=${form.current.search.value}`).then(r=> setUsers(r));
  }
  ...
  return(
    <div className="container-fluid">
      <Welcome text='Usuarios' add={()=> setModalShow(true)}/>
