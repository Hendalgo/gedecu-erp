...
export function validateFields(formData = new FormData()) {
  ...
  if (formData.has("wallet") && !formData.get("wallet")) errors.push("El campo Billetera es obligatorio.");

  if (formData.has("user") && !formData.get("user")) errors.push("El campo Gestor es obligatorio.");

  if (formData.get("amount") == "0,00") errors.push("El campo Monto es obligatorio.");

  if (formData.has("rate") && formData.get("rate") == "0,00") errors.push("El campo Tasa es obligatorio.");
