  ...
  const handleReport = async () => {
    try {
      ...
      formData.payment_reference
        ? (data.payment_reference = formData.payment_reference.value)
        : null;
      formData.store.value ? (data.store = formData.store.value) : null;
      formData.bank && formData.bank.value !== ""
        ? (data.bank = formData.bank.value)
        : null;
    ...
    getReportTypes(`paginated=no`).then((r) => {
      if (Array.isArray(r)) {
        const income = r
          ...
          .map((e) => ({ value: e.id, label: e.name, type: e.type }));
        const expense = r
          .filter((e) => e.type === "expense")
          .map((e) => ({ value: e.id, label: e.name, type: e.type }));
        const neutro = r
          .filter((e) => e.type === "neutro")
          .map((e) => ({ value: e.id, label: e.name, type: e.type }));
            ...
            <div className="row">
              <div className="d-flex flex-column">
                ...
                <span className="ModalTopSubTitle">
                  En esta pestaña puedes crear un reporte nuevo. Los tipos de
                  reporte se dividen en Ingreso, egreso o neutral. Las de tipo
                  ingreso o egreso agregan el campo "Cuenta de ingreso/debitar"
                  el cual implica a que cuenta se debitaran o agregaran los
                  fondos de la operacion y la moneda en que se basará la tasa y
                  el monto. Si marcas el movimiento como duplicado la operación
                  se realizará 2 veces. Ejemplo si haces un movimiento de tipo
                  egreso de $5. A la cuenta se debitaran $10.
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="container">
            ...
            <div className="row mt-3">
              ...
              {options.payment_reference && (
                <div className="col-4">
                  <label htmlFor="payment_reference" className="form-label">
                    Referencia de pago
                  </label>
                  <input
                    type="text"
