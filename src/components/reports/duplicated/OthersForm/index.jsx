...
export default function OthersForm() {
    ...
    const handleCurrencyChange = async (option) => {
        ...
        try {
            ...
            setStores(storesResponse.data.map(({id, name}) => ({label: `${name}`, value: id})));
        } catch (error) {
            console.log(error)
        }
        ...
        <>
            ...
            <div className="row mb-3">
                <div className="col">
                    ...
                    <Select isDisabled={stores.length === 0} inputId="store_id" name="store_id" options={stores} value={store} onChange={handleStoreChange} placeholder="Seleccione el local" noOptionsMessage={() => "No hay coincidencias"} />
