...
const TypeTwoTransferReportForm = () => { // Reporte de traspaso Tipo 2
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setError({ show: true, message: [response.data.message], danger: "danger", });
            }
        }
        fetchData();
    }, [])
    ...
    const handleSenderAccountChange = (option) => {
        if (selectedReceiverAccount && option && selectedReceiverAccount.currency_id !== option.currency_id) {
            setError({ show: true, message: ["Las cuentas deben implementar la misma moneda."], variant: "danger", });
        } else {
            const newOptions = getOptions(bankAccounts.current.filter(({ id }) => id != option?.value || !option));
    ...
    const handleReceiverAccountChange = (option) => {
        if (selectedSenderAccount && option && selectedSenderAccount.currency_id !== option.currency_id) {
            setError({ show: true, message: ["Las cuentas deben implementar la misma moneda."], variant: "danger", });
        } else {
            const newOptions = getOptions(bankAccounts.current.filter(({ id }) => id != option?.value || !option));
    ...
    const handleLocalSubmit = (e) => {
        ...
        try {
            ...
            if ((selectedSenderAccount && selectedReceiverAccount) && selectedSenderAccount.currency_id !== selectedReceiverAccount.currency_id) errors.push("Las cuentas deben implementar la misma moneda.");
    ...
    return(
        <form onSubmit={handleLocalSubmit} onReset={handleReset} autoComplete="off">
            <div className="row mb-3">
                <div className="col">
                    <input type="hidden" id="senderAccount" name="senderAccount" value={selectedSenderAccount?.label || ""} />
                ...
                <div className="col">
                    <input type="hidden" id="receiverAccount" name="receiverAccount" value={selectedReceiverAccount?.label || ""} />
            ...
            <input type="hidden" name="currency_id" value={selectedSenderAccount?.currency_id || 0} />
            <input type="hidden" name="currency" value={selectedSenderAccount?.currency || ""} />
            <div className="row mb-3">
                <div className="col-6">
