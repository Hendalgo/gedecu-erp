...
const SideBar = ({ children }) => {
  ...
  return width > 1440 ? (
    <SideBarBig menus={menus} setSession={setSession} isActive={isActive}>
      {children}
    </SideBarBig>
  ) : (
    <SideBarSmall menus={menus} setSession={setSession} isActive={isActive}>
      {children}
    </SideBarSmall>
  );
};
        ...
        <div className="col-3 SideBarCol d-flex flex-column flex-shrink-0 p-3">
          ...
          <ul className="nav nav-pills flex-column mb-auto">
            {isActive.map((Menu, index) => (
              <li key={index} className="SideBarItem p-1">
                {!Menu.others ? (
                    ...
                    >
                      ...
                      <Accordion.Body>
                        {Menu.others.map((e, index) => (
                          <NavLink className="nav-link" to={e.link} key={index}>
                            {e.name}
                          </NavLink>
                        ))}
                      </Accordion.Body>
